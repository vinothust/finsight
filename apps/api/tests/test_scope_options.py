...
def _seed(db_session):
    ...
    a1 = Account(name="Zeta Corp", cluster_id=cluster.id)
    a2 = Account(name="Acme Corp", cluster_id=cluster.id)
    db_session.add_all([a1, a2])
    db_session.flush()

    p1 = Project(name="Beta Program", account_id=a1.id)
    p2 = Project(name="Alpha Program", account_id=a2.id)
    p3 = Project(name="Zed Program", account_id=a1.id)
...
def test_list_projects_without_account_id_returns_all_projects_ordered_by_name(client, db_session):
    _seed(db_session)
    response = client.get("/scope-options/projects")
    assert response.status_code == 200
    body = response.json()
...
def test_list_projects_with_account_id_filters_correctly(client, db_session):
    a1, a2, p1, p2, p3 = _seed(db_session)
    response = client.get(f"/scope-options/projects?account_id={a1.id}")
    assert response.status_code == 200
    body = response.json()
