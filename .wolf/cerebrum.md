...
- **ScopePicker (Task 16):** `apps/web/src/components/ScopePicker.tsx` fixes the previously-dead `scopeId` — it's now real `useState` in `RoleContext` with a `setScopeId` setter, reset to `null` whenever `setRole` runs (account_director's scope = account, pm's scope = program — different option sets, so a stale id from the other role would be meaningless). `ScopePicker` renders nothing for `area_director`, an account `Select` for `account_director` (via `getAccountOptions()`), and a program `Select` for `pm` (via `getProgramOptions()`), both hitting the Task 15 `/scope-options/*` endpoints unscoped (no role/scope headers — those endpoints are deliberately unfiltered pickers). `scopeId` intentionally has NO localStorage persistence (resets on reload) — descoped on purpose per the task-16 brief, do not add it back.
